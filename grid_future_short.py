...
if len(sys.argv) < 2:
    ...
    module_name = "BTCUSDC_future_short_header"
else:
    module_name = sys.argv[1]+"_future_short_header"
...
control_file = CurrentSymbol+"_future.ctl"
io_file = CurrentSymbol+"_future.out"
...
while (True):
    ...
    try:
        ...
        if (order['status'] == 'FILLED'):
            ...
            print("Initial SELL order filled, qty=%.4f, price=%.4f, CostForPosition=%.4f" %(quantity_to_sell, price_to_sell, CostForPosition) )
...
for i in range(NumberOfTrailingDownGrids+NumberOfInitialBuyGrids, NumberOfTrailingDownGrids+NumberOfInitialBuyGrids+NumberOfInitialSellGrids):
    ...
    order = client.futures_create_order(symbol=CurrentSymbol, side=client.SIDE_SELL, type='LIMIT', quantity=QtyPerOrder, price=price_to_sell, timeInForce="GTC")
...
### 3 Placing Initial Buy Order (closing postion)
for i in range(NumberOfTrailingDownGrids, NumberOfTrailingDownGrids + NumberOfInitialBuyGrids ):
...
### 4 Placing Initial Buying Dip Orders
...
while (True):
    ...
    for i in range(NumberOfTotalGrids):
        ...
        if (order['status'] == 'FILLED'):
            ...
            if (GridTradeNodeList[i].order_status == OrderStatus_SellOrderPlaced):
                ...
                print("CostForPosition+=%.4f, CostForPosition =%.4f, GridProfit = %.4f " %(QtyPerOrder * GridTradeNodeList[i].sell_order_executed_price,CostForPosition, GridProfit))
                ...
                try:
                    ...
                    print("Position=%.4f, current_price=%.4f, current_position_value =%.4f "
                ...
                print("%s %d (%d):  BUY Order Filled at %.2f, Profit +++++++ placing a new SELL at --> %.2f ."
                ...
                Position += QtyPerOrder
                print("Before, CostForPosition = %.4f, GridProfit = %.4f" %(CostForPosition, GridProfit))
                CostForPosition -= QtyPerOrder *  GridTradeNodeList[i].sell_order_executed_price
                ...
                print("Grid profit += %.4f" %((GridTradeNodeList[i].sell_order_executed_price - GridTradeNodeList[i].buy_order_executed_price)* QtyPerOrder))
                print("CostForPosition-=%.4f, after, CostForPosition =%.4f, GridProfit = %.4f " %(QtyPerOrder *  GridTradeNodeList[i].sell_order_executed_price,CostForPosition, GridProfit))
                ...
                try:
                    ...
                    print("%s Matched times:%d  Grid Profit is %.4f, FloatingProfit is %.4f, Total Profit is %.4f, position is %.4f \n"
                        % (current_time,matched_number, GridProfit, FloatingProfit, TotalProfit, Position))
                except:
                    print(traceback.format_exc())
                    print("%s Failed to get current price" %(current_time) );
    ...
    if ( current_price <  trail_down_trigger_price  and  (NumberOfTrailingDownGrids + n_trail_up_or_down) > 0 ):
        ...
        #The market sell is counted to the new lowest active node., we suppose it is sold at current_price
        lowest_index= NumberOfTrailingDownGrids + n_trail_up_or_down
        GridTradeNodeList[lowest_index-1].sell_order_executed_price = price_to_sell
        Position -= QtyPerOrder

        print("Before traildown, CostForPosition = %.4f, GridProfit = %.4f" %(CostForPosition, GridProfit))

        CostForPosition += QtyPerOrder * GridTradeNodeList[lowest_index-1].sell_order_executed_price

        print("Traildown, cost for new grid is counted to %.4f" %(GridTradeNodeList[lowest_index-1].sell_order_executed_price))
        print("After traildown, CostForPosition+=%.4f CostForPosition = %.4f, GridProfit = %.4f" 
        %(QtyPerOrder * GridTradeNodeList[lowest_index-1].sell_order_executed_price, CostForPosition, GridProfit))
        ...
        #print("%s: TrailDown, the grid profit for market sold highest trade is %.4f, sold price supposed to be %.4f " %(current_time, profit_for_market_sell,current_price ))
        ...
        #Grid Profit for short, is counted to lowest grid, although it is INACTIVE


        print("Before trail UP , CostForPosition = %.4f, GridProfit = %.4f" %(CostForPosition, GridProfit))

        GridTradeNodeList[lowest_index].buy_order_executed_price = current_price
        profit_for_market_buy = (GridTradeNodeList[lowest_index].sell_order_executed_price - GridTradeNodeList[lowest_index].buy_order_executed_price)* QtyPerOrder
        GridProfit = GridProfit + profit_for_market_buy
        Position += QtyPerOrder
        CostForPosition -= QtyPerOrder * GridTradeNodeList[lowest_index].sell_order_executed_price

        print("Trail up, cost for the canceled grid is counted to %.4f"  %(QtyPerOrder * GridTradeNodeList[lowest_index].sell_order_executed_price))

        print("After trail up, CostForPosition-=%.4f CostForPosition = %.4f, GridProfit+=%.4f, GridProfit = %.4f" 
        %( QtyPerOrder * GridTradeNodeList[lowest_index].sell_order_executed_price, CostForPosition,profit_for_market_buy, GridProfit))
        FloatingProfit = CostForPosition -current_price * (-1*Position)
        TotalProfit = FloatingProfit + GridProfit
